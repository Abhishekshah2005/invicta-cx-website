import { cn } from "@/lib/utils";

type Tone = "ink" | "ivory" | "crimson";

interface ImagePlaceholderProps {
  /** Description of the intended real photograph (shown as a caption). */
  label: string;
  /** CSS aspect-ratio, e.g. "4 / 5", "16 / 9". */
  ratio?: string;
  /** Visual register. */
  tone?: Tone;
  className?: string;
}

const TONE: Record<Tone, string> = {
  ink: "bg-foreground text-background",
  ivory: "bg-muted text-foreground",
  crimson: "bg-brand text-brand-foreground",
};

/**
 * A branded stand-in for real photography. Renders an editorial duotone panel
 * with grain and a caption describing the intended shot, so layouts read as
 * intentional (not broken) before assets land. Swap for `<BlurImage>` at
 * asset-integration time — the aspect ratio and rounding are already correct.
 *
 * ART DIRECTION: real people · premium/cinematic lighting · human support teams,
 * gaming operations, enterprise meetings. No AI illustration, no stocky look.
 */
export function ImagePlaceholder({
  label,
  ratio = "4 / 5",
  tone = "ink",
  className,
}: ImagePlaceholderProps) {
  return (
    <div
      role="img"
      aria-label={`Photography placeholder: ${label}`}
      className={cn("relative isolate overflow-hidden rounded-2xl", TONE[tone], className)}
      style={{ aspectRatio: ratio }}
    >
      {/* Depth: soft radial + film grain */}
      <div
        aria-hidden
        className="absolute inset-0 opacity-60"
        style={{
          backgroundImage:
            "radial-gradient(120% 80% at 30% 0%, color-mix(in oklab, currentColor 14%, transparent), transparent 60%)",
        }}
      />
      <div
        aria-hidden
        className="absolute inset-0 noise-overlay opacity-[0.06] mix-blend-overlay"
      />

      <div className="absolute inset-0 flex flex-col justify-between p-5">
        <span className="font-mono text-[0.65rem] tracking-[0.22em] uppercase opacity-70">
          Photography
        </span>
        <span className="max-w-[85%] font-mono text-xs leading-relaxed tracking-wide opacity-90">
          {label}
        </span>
      </div>
    </div>
  );
}
